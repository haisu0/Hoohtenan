...
async def handle_downloader(event, client):
    ...
    # Validasi akses ke target_chat (untuk private, channel, grup)
    try:
        await client.get_entity(target_chat)
    except Exception as e:
        await event.reply(f"❌ Tidak bisa akses chat {target_chat}. Pastikan chat valid dan akun bisa kirim pesan ke sana. Error: {str(e)}")
        return
