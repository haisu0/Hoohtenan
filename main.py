...
async def process_downloader_link(event, client, url, send_to):
    # resolve target chat
    if isinstance(send_to, (int, str)):
        try:
            send_to = await client.get_entity(send_to)
        except Exception as e:
            await event.reply(f"❌ Tidak bisa kirim ke {send_to}: {e}")
            return
...
async def handle_downloader_multi(event, client):
    ...
    if args:
        ...
        if re.match(r'^@?[a-zA-Z0-9_]+$', first_arg) or re.match(r'^-?\d+$', first_arg):
            ...
            # argumen pertama bukan chatid → langsung treat sebagai link
